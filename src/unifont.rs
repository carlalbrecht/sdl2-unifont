...
/// Unifont files.
static mut UNIFONT: Option<Box<Mutex<HashMap<u32, FontChar>>>> = None;
...
fn initialise_generic(font: &[u8]) -> Result<(), LzmaError> {
    ...
    let mut hashmap = unsafe {
        match UNIFONT {
            ...
            Some(ref mut font_lock) => font_lock.lock().unwrap(),
        }
    };
...
fn initialise_unifont() -> Result<(), LzmaError> {
    ...
    unsafe {
        UNIFONT = Some(Box::new(Mutex::new(HashMap::new())));
    }
...
/// said structure if it hasn't already been. Note that the result will need to
/// be dereferenced with `*`, since it is wrapped in a `MutexGuard`.
pub fn get_unifont<'a>(
) -> Result<MutexGuard<'a, HashMap<u32, FontChar>>, LzmaError> {
    unsafe {
        match UNIFONT {
            ...
            Some(ref font_lock) => Ok(font_lock.lock().unwrap()),
        }
    }
}
