...
/// Unifont files.
static mut UNIFONT: Option<Box<Mutex<FontChars>>> = None;
...
/// said structure if it hasn't already been. Since the Unifont is backed by a
/// mutex, only one reference can exist at any time. Therefore, care must be
/// taken to avoid deadlocks.
pub fn get_unifont<'a>() -> Result<MutexGuard<'a, FontChars>, LzmaError> {
    unsafe {
        match UNIFONT {
